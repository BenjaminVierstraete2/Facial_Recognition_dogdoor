...
#check if txt file has multiple bounding boxes
toberemoved = []
for i in range(len(faces)):
    if len(faces[i]) > 1:
        print(f"image {i} has multiple bounding boxes")
        toberemoved.append(i)
#delete 
for i in range(len(toberemoved)):
    del faces[toberemoved[i]-i]
    del images[toberemoved[i]-i]



for i in range(len(faces)):
    if len(faces[i]) > 1:
        print(f"image {i} still has multiple bounding boxes")
